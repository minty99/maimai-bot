...
CREATE TABLE scores (
  ...
  last_played_at TEXT,
  play_count INTEGER,
  PRIMARY KEY (title, chart_type, diff_category, genre, artist)
);

CREATE INDEX IF NOT EXISTS idx_scores_title ON scores(title);
CREATE INDEX IF NOT EXISTS idx_scores_identity ON scores(title, genre, artist);
...
CREATE INDEX IF NOT EXISTS idx_playlogs_song_identity ON playlogs(title, genre, artist, chart_type, diff_category);
